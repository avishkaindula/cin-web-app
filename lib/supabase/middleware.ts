...
const hasRequiredPrivileges = (
  ...
  return requiredPrivileges.every((required) =>
    userPrivileges.some(
      (priv) => priv.type === required && priv.status === "approved"
    )
  );
};
  ...
  try {
    ...
    if (session?.access_token) {
      try {
        ...
        isCinAdmin = userPrivileges.some(
          (priv: UserPrivilege) =>
    ...
    if (!user.error && (isCinAdmin || isAdmin)) {
      ...
      if (routePermission) {
        ...
        const hasPrivileges = hasRequiredPrivileges(
          userPrivileges,
          routePermission.privileges
        );
        ...
        if (!hasPrivileges) {
          ...
          redirectUrl.searchParams.set(
            "required_privileges",
            routePermission.privileges.join(",")
          );
          return NextResponse.redirect(redirectUrl);
        }
      }
